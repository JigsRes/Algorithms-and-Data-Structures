...
class rbtree(object):
    ...
    def _insert_fixup(self, z):
        ...
        while z.p.red:  #if violation of red-red
            if z.p == z.p.p.left:  ## if left sided
                y = z.p.p.right  #y is uncle
                if y.red:     #Case 1
                    z.p._red = False
                    y._red = False
                    z.p.p._red = True
                    z = z.p.p      #z changed to grandparent
                else:
                    if z == z.p.right:   #Case 2
                        z = z.p   #Roate w.r.t parent
                        self._left_rotate(z)
                    z.p._red = False    #Case 3
                    z.p.p._red = True
                    self._right_rotate(z.p.p)   #Roate w.r.t grand-parent
            else:     ##if right sided
                y = z.p.p.left
                if y.red:  #Case 1
                    z.p._red = False
    ...
    def _left_rotate(self, x):
        "Left rotate x."
        y = x.right  #get right node in y
        x._right = y.left   #set right property of x to  y.
        if y.left != self.nil:
            y.left._p = x
        y._p = x.p
        if x.p == self.nil:
